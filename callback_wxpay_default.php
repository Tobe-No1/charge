...
$pay_object = new wxpay($data['attach']);
$check_sign = $pay_object->signverify($data);
write_callback_file('wxpay','network success');
if (!$check_sign) {
...
$charge_info = $db->get_one(sprintf("select * from charge where charge_id = %d", $cpOrderId));
...
if ($charge_info['amount'] > $data['total_fee']) {
    write_callback_file('wxpay','error amount');
...
$re = $db->update_new('charge', "charge_id=" . $charge_info['charge_id'], $update_data);
if($re){
